...
function formatYtDlpError(stderrText = '') {
  const err = String(stderrText || '').trim();
  const low = err.toLowerCase();

  const needsLogin =
    low.includes('you need to log in') ||
    low.includes('use --cookies') ||
    low.includes('authentication') ||
    low.includes('[instagram:story]');

  if (needsLogin) {
    return 'Stories nie działają bez pliku cookies (ani publiczne, ani prywatne). Bez cookies działa tylko publiczny content (posty, Reels). W Ustawieniach wskaż cookies.txt z zalogowanej sesji Instagram.';
  }

  if (!err) return 'Brak danych z yt-dlp';
...
function fetchInfo(url, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', () => {
      if (!out.trim()) return reject(new Error(formatYtDlpError(err)));
      try {
        resolve(out.trim().split('\n').filter(Boolean).map(l => JSON.parse(l)));
      } catch {
        reject(new Error('Nie udało się odczytać metadanych z yt-dlp'));
      }
    });
